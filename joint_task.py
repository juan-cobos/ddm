...
def plot_comparison(results_base, results_coup, window=20):
    ...
    ax[0].plot(vm2_b[:, 0] - vm2_b[:, 1], color="C0", alpha=0.4, linestyle="--", label="Uncoupled M2")
    ax[0].plot(vm1_c[:, 0] - vm1_c[:, 1], color="C1", alpha=0.8, label="Coupled M1")
    ax[0].plot(vm2_c[:, 0] - vm2_c[:, 1], color="C1", alpha=0.4, linestyle="--", label="Coupled M2")
    ...
    # Side agreement rate
    ma_b = np.convolve((np.array(sides1_b) == np.array(sides2_b)).astype(float), np.ones(window) / window, mode="valid")
    ma_c = np.convolve((np.array(sides1_c) == np.array(sides2_c)).astype(float), np.ones(window) / window, mode="valid")
