...
for _ in range(steps):
    ...
    # Reward rule
    if abs(model1.t - model2.t) <=  time_window and np.sign(model1.X) == np.sign(model2.X):
        joint_rewards += 1
    ...
    print(f"Values model1: [{model1.v1:.3f}][{model1.v2:.3f}] ------ Values model2: [{model2.v1:.3f}][{model2.v2:.3f}]")
...
xlim = 20000
xstep = 2000
ax[0].set_xticks(np.arange(0, xlim+1, xstep), np.arange(0, xlim+1, xstep), fontsize=14)
...
h = ax2.hist2d(value_map1[0], value_map1[1], bins=100, range=[[0, 1], [0, 1]], density=True, label="Model1")
