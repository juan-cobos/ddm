...
class DDM:
    ...
    def run(self, n_trials=100, plot=False):
        ...
        while trial < n_trials:
            ...
            # Set boundaries
            def compute_bounds():
                bound = self.bound_scaling * np.exp(
                    -self.decay_rate * (self.v_left + self.v_right) * self.t * self.dt
                )
                return bound, -bound
            ...
            if (
                self.relative_value_signal >= upper_bound
                or self.relative_value_signal <= lower_bound
            ):  # If it crosses the threshold

                def update_value():
                    if (
                        self.time_range[0] <= self.t <= self.time_range[1]
                    ):  # If within time_range
                        reward = 1
                        self.n_rewards += 1
                    ...
                    if self.relative_value_signal >= upper_bound:  # Chose left action
                        ...
                        self.relative_value_signal <= lower_bound
                    ):  # Chose right action
        ...
        if plot:
            ...
            ax[0].set_xticks(
                np.arange(0, max_rt, round(max_rt / 10)),
                labels=np.arange(0, max_rt, round(max_rt / 10)),
            )
